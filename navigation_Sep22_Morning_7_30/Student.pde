class Student {
  ...
  String getallcafestatus () {
    String cafeUsage, InternUsage;

    cafeUsage =  row.getString ("CAFEE");
    InternUsage = row.getString("INTERNSHIP");
    if (cafeUsage.equals("1")) {
      if (InternUsage.equals("1")) {
        cafeUsage = "Cafe User (N)";
      } else if (InternUsage.equals("2")) {
        cafeUsage = "Cafe User (Sp.)";
      } else if (InternUsage.equals("3")) {
        cafeUsage = "Cafe User (OB)";
      }
  ...
  boolean hadMeal () {
    return hadBreakfast () || hadLunch () || hadDinner ();
  }
